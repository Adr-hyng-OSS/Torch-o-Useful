...
class Stream extends DataView {
    ...
    ;
    set __offset__(v) { if (v > this.byteLength)
        throw new EndOfStreamError("Offset cant be out of Stream."); this.__off_set__ = v; if (v > this.__size__)
        this.__size__ = v; }
    ...
    static fromString(text, options, ...params) {
        ...
        if (text.length * BYTES_PER_ELEMENT > bufferLength)
            throw new RangeError("Can't fit text to specified buffer, please increase buffer length");
        const typedArray = new DataView(new ArrayBuffer(bufferLength));
        if (!property in typedArray)
            throw new Error("Invalid base: " + base);
        for (let i = 0; i < text.length; i++)
            typedArray[property](i * BYTES_PER_ELEMENT, String.prototype.charCodeAt.call(text, i) - 1);
...
class NBTStreamReader extends BinaryStreamReader {
    ...
    readCompoud() {
        ...
        while (true) {
            ...
            if (dataType === this.__options__.nbtTypes.EndOfCompoud)
                break;
...
class NBT {
    ...
    static getType(object) {
        switch (typeof object) {
            ...
            case "number":
                if (isUint8(object))
                    return NBTTypes.Uint8;
                if (isUint16(object))
                    return NBTTypes.Uint16;
                if (isInt32(object))
                    return NBTTypes.Int32;
